...
1.  **Reset**: Re-populates the emoji pool.
2.  **Distribute**: Iterates through the `Children` collection of the layout, filtering specifically for `Button` types using LINQ.
3.  **Shuffle**: Assigns random emojis and removes them from the pool to ensure perfect pairs.

## 🚦 Getting Started

1.  **Prerequisites**: Visual Studio 2022 with the ".NET Multi-platform App UI development" workload installed.
2.  **Clone**: Open the project folder in Visual Studio.
3.  **Run**: Select your target platform (Windows, Android, or iOS) and press **F5**.
